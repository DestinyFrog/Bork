...
export class Draw {
    static Background(color) {
        ctx.fillStyle = color || game["STANDARD"].color;
        ctx.fillRect(0, 0, game.WIDTH, game.HEIGHT);
    }
    static BordBackground(color, width) {
        ctx.strokeStyle = color || game["STANDARD"].color;
        ctx.lineWidth = width || game["STANDARD"].width;
    ...
    static Write(position, message, size, color, font, align, base) {
        ...
        ctx.fillStyle = color || game["STANDARD"].color;
        ctx.font = ((size || game["STANDARD"].fontSize).toString()) + "px " + (font || game["STANDARD"].font);
